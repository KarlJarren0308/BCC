            ...
            <table id="books-table" class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach($books as $book)
                        <tr>
                            <td>{{ 'C' . sprintf('%04d', $book->Accession_Number) }}</td>
                            <td>{{ $book->Title }}</td>
                            <td>{{ $book->Edition }}</td>
                            ...
                            <td class="text-center">
                                @if(session()->has('username'))
                                    <button data-button="receive-book-button" data-var-id="{{ $book->Book_ID }}" class="btn btn-primary btn-xs">Receive Book</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
