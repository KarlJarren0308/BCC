...
@section('content')
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Manage Holidays</h1>
                </div>
            </div>
            ...
            <div class="text-right gap-bottom">
                <a href="{{ route('dashboard.getAddRecord', 'holidays') }}" class="btn btn-primary">Add Holiday</a>
            </div>
                ...
                <tbody>
                    @foreach($holidays as $holiday)
                        <tr>
                            ...
                            <td>{{ date('F d, Y', strtotime($holiday->Date_Stamp)) }}</td>
                            <td>{{ $holiday->Type }}</td>
                            <td class="text-center">
                                @if(session()->has('username'))
                                    <a href="{{ route('dashboard.getEditRecord', ['holidays', $holiday->Holiday_ID]) }}" class="btn btn-success btn-xs">Edit</a>
                                    <a href="{{ route('dashboard.getDeleteRecord', ['holidays', $holiday->Holiday_ID]) }}" class="btn btn-danger btn-xs">Delete</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
