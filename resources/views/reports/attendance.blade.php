            ...
            <tbody>
                @foreach($borrowers as $borrower)
                    @if(strlen($borrower->Middle_Name) > 1)
                        <?php $name = $borrower->First_Name . ' ' . substr($borrower->Middle_Name, 0, 1) . '. ' . $borrower->Last_Name; ?>
                    @else
                        <?php $name = $borrower->First_Name . ' ' . $borrower->Last_Name; ?>
                    @endif
                    <tr>
                        <td class="text-center">{{ $borrower->Username }}</td>
                        <td class="text-center">{{ $name }}</td>
                        <td class="text-center">{{ date('F d, Y', strtotime($borrower->Date_Stamp)) }}</td>
                        <td class="text-center">{{ date('h:i A', strtotime($borrower->Time_Stamp)) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
